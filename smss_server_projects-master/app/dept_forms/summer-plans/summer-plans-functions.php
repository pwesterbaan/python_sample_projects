...
$admin_list = array('HEDETNI','JDMCKEN','KHAN','LCALLA','AHAYNE','JDYKEN','NATNAT','VMCCLAI','KEVJA','PGERARD','SSATHER');

function get_nav()
{
	global $admin_list;
	global $user_id;
	$nav = '<li><a href="index.php">Summer Plans Entry</a></li>';
	if (in_array($user_id,$admin_list))
	{
		$nav .= '<li><a href="admin-view-entries.php">Admin View Entries</a></li>';
		$nav .= '<li><a href="admin-settings.php">Admin Settings</a></li>';
	}
	return $nav;
}
...
{
	global $mthsc_db;
	$query = $mthsc_db->query('SELECT * FROM gs_info.student_profile INNER JOIN dept_info.person on student_profile.person_id = person.person_id WHERE status="Enrolled" OR status="Temporary Leave" OR status="Incoming" ORDER BY last_name ASC');
	return $query->fetchAll();
}
