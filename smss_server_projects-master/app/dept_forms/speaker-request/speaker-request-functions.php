...
$admin_list = array('hedetni','kevja','lcalla','jdbible');

//this list deterines who gets emailed when a request is submitted
$notification_list = array('lcalla@clemson.edu','jdbible@clemson.edu');
//$notification_list = array('hedetni@clemson.edu'); //for testing

//this is who the confirmation emails come from
$research_chair = 'jdbible@clemson.edu';

function get_nav()
{
	global $admin_list;
	global $user_id;
	$nav = '<li><a href="index.php">Submit New Speaker Request</a></li>';
	$nav .= '<li><a href="view-talks.php">View Scheduled Talks</a></li>';
	if (in_array($user_id,$admin_list))
	{
		$nav .= '<li><a href="admin-view-requests.php">Admin View Speaker Requests</a></li>';
	}
	return $nav;
}
...
{
	global $mthsc_db;
	$get_comments_query = $mthsc_db->prepare('SELECT * FROM speaker_request_comments WHERE request_id = ?;');
