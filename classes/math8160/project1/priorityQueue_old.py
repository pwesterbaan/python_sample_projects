...
#         print(H1.get_element().edge(),H2.get_element().edge())
        if H1 == None:
            return H2
        elif H2 == None:
            return H1
        elif H1 < H2:
            minheap, maxheap = H1, H2
        else:
            minheap, maxheap = H2, H1
        #get right branch of minheap
        rightBranch = minheap.get_rightChild()
        if rightBranch != None:
            rightBranch=self.meld(rightBranch, maxheap)
    ...
    def merge(self,mpq,new_node):
        """TODO: Based of meld? Do we need this?"""
#         if not isinstance(new_node, self._Items):
#             new_node=self.make_mpq(new_node)
        mpq=self.meld(mpq, new_node)